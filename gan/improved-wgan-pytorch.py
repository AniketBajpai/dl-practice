...
# generator_opt = optim.Adam(generator_model.parameters(), lr=generator_lr, betas=(0.5, 0.9))
# discriminator_opt = optim.Adam(discriminator_model.parameters(), lr=discriminator_lr, betas=(0.5, 0.9))
generator_opt = optim.RMSprop(generator_model.parameters(), lr=generator_lr)
discriminator_opt = optim.RMSprop(discriminator_model.parameters(), lr=discriminator_lr)
...
for it in range(num_epochs):
    for _ in range(5):
        ...
        x = torch.add(torch.mul(X, epsilon), torch.mul(G_sample, (1.0-epsilon)))
        d_x = discriminator_model(x)
        # g = torch.ones(d_x.size())
        # grad = torch.autograd.grad(d_x, x, g)
        gradient_penalty = torch.pow(torch.norm(grad) - 1, 2)

        discriminator_loss = -(torch.mean(D_real) - torch.mean(D_fake)) + loss_lambda * gradient_penalty
